...
const pjPrefix = 'BLEAFSI-LogBase';
...
describe(`${pjPrefix} Compare Snapshot test`, () => {
  test('LoggingAccount Stacks', () => {
    //create stacks
    stack = new BaseCTStack(app, `${pjPrefix}-Dev`, DevParameter);
...
describe(`${pjPrefix} cdk-nag AwsSolutions Pack`, () => {
  beforeAll(() => {
    ...
    NagSuppressions.addResourceSuppressionsByPath(stack, '/BLEAFSI-LogBase-Dev/Bucket/AccessLogs/Default/Resource', [
      {
        id: 'AwsSolutions-S1',
