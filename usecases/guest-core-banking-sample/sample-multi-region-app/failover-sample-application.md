# マルチリージョン対応 マイクロサービス サンプルアプリケーションのフェールオーバー手順

マルチリージョン マイクロサービス・アプリケーションを東京リージョンから大阪リージョンにフェールオーバーさせるための手順について解説します。
...
- Aurora の Planned Failover を実施し、Aurora グローバルクラスタを東京リージョンから大阪リージョンへ切り替えます  
  ...
  - [アクション]から[グローバルデータベースをフェールオーバー]を選択

    <img src="./imgs/failover-001.png" width="50%">

  - フェールオーバーが開始されるので、完了するまで待つ
