# マルチリージョン対応 マイクロサービス サンプルアプリケーションの利用手順
...
アプリケーションは下記の 4 つのマイクロサービス(ECS コンテナ)から構成され、Saga パターンによる保証トランザクションの仕組みを実装することで、分散環境でのトランザクションの整合性を保持するように実装されています。
...
## サンプルアプリケーションをフェールオーバーさせる

本アプリケーションを東京リージョンから大阪リージョンにフェールオーバーする手順については[こちら](./failover-sample-application.md)を参照して下さい。
